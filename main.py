...
INITIAL_ADMIN_ID = int(os.getenv("INITIAL_ADMIN_ID", 0))  # Asosiy admin ID
...
async def create_db_pool():
    ...
    if pool is None:
        try:
            ...
            logger.error(f"Database pool yaratishda xato: {e}")
            raise
...
async def save_user(user_id: int, username: str = None):
    ...
    try:
        ...
        logger.error(f"Foydalanuvchini saqlashda xato (ID: {user_id}): {e}")
...
async def log_user_activity(user_id: int, username: str, activity_type: str):
    ...
    try:
        ...
        logger.error(f"Faollikni loglashda xato (ID: {user_id}): {e}")
...
async def deactivate_user(user_id: int):
    ...
    try:
        ...
        logger.error(f"Foydalanuvchini deaktivatsiya qilishda xato (ID: {user_id}): {e}")
...
async def is_admin(user_id: int) -> bool:
    ...
    try:
        ...
        logger.error(f"Admin tekshiruvida xato (ID: {user_id}): {e}")
        return False
...
async def handle_start(message: Message):
    try:
        ...
        logger.error(f"Start handlerida xato: {e}")
...
async def handle_stats_command(message: Message):
    try:
        ...
        async with pool.acquire() as conn:
            ...
            if top_user:
                stats_text += (
                    f"🏆 Eng faol foydalanuvchi (7 kun):\n"
                    f"👤 {top_user['username'] or 'Noma\'lum'}\n"
                    f"📊 Faollik: {top_user['activity_count']} marta\n"
                )
                
        await message.answer(stats_text, parse_mode=ParseMode.HTML, reply_markup=admin_keyboard)
...
async def handle_dump_users(message: Message):
    try:
        ...
        logger.error(f"Userlar ro'yxatini yuklashda xato: {e}")
        await message.answer(
...
async def handle_photo(message: Message):
    try:
        ...
        logger.error(f"Rasmli xabar handlerida xato: {e}")
        await message.answer(random.choice(error_messages))
...
async def notify_inactive_users():
    while True:
        ...
        try:
            ...
            logger.error(f"Notifikatsiya yuborishda xato: {e}")

async def main():
    try:
        # Database ulanishini o'rnatish
        await create_db_pool()
        ...
        async with pool.acquire() as conn:
            ...
            logger.info(f"Dastlabki adminlar: {admins}")
        ...
        # Notifikatsiya taskini ishga tushirish
        asyncio.create_task(notify_inactive_users())
        
        await dp.start_polling(bot)
        
    except Exception as e:
        logger.critical(f"Bot ishga tushirishda kritik xato: {e}")
    finally:
        if pool:
            await pool.close()
